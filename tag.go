...
const (
	...
	// LogEvent A stable identifier for some notable moment in the lifetime of a Span. For instance, a mutex lock acquisition or release or the sorts of lifetime events in a browser page load described in the Performance.timing specification.
...
func Int64(key string, val int64) Tag {
	return tagInt64(key, val)
}

// Float32 new tag Float32.
func Float32(key string, val float32) Tag {
	return tagFloat32(key, val)
}

// Float64 new tag Float64.
func Float64(key string, val float64) Tag {
	return tagFloat64(key, val)
}
